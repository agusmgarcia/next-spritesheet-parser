...
> {
    ...
    this.subscribe(
      (state) => ({
        ...
        sprites: state.sprites.map((s) => ({
          center: { offsetX: s.center.offsetX, offsetY: s.center.offsetY },
          id: s.id,
        })),
      }),
      ({ id, ...props }) =>
  ...
  get enableCenterDisabled(): boolean {
    const center = this.state.sprites.at(this.state.index)?.center;
    return this.state.playing || !center || center.visible;
  }

  enableCenter(): void {
    if (this.enableCenterDisabled) return;
    this.state = {
      ...this.state,
      sprites: this.state.sprites.map((sprite, index) =>
        index === this.state.index
          ? { ...sprite, center: { ...sprite.center, visible: true } }
          : sprite,
      ),
    };
  }

  get disableCenterDisabled(): boolean {
    const center = this.state.sprites.at(this.state.index)?.center;
    return this.state.playing || !center || !center.visible;
  }

  disableCenter(): void {
    if (this.disableCenterDisabled) return;
    this.state = {
      ...this.state,
      sprites: this.state.sprites.map((sprite, index) =>
        index === this.state.index
          ? { ...sprite, center: { ...sprite.center, visible: false } }
          : sprite,
      ),
    };
  }
