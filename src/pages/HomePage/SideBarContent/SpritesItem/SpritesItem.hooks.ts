...
export default function useSpritesItem(props: SpritesItemProps) {
  ...
  const { deleteSpritesDisabled, deleteSpritesOnClick } = useDeleteSprites();
  ...
  return {
    ...props,
    defaultCollapsed,
    deleteSpritesDisabled,
    deleteSpritesOnClick,
    disabled,
...
function useDeleteSprites() {
  const { spriteSelection } = useSpriteSelection();
  const { deleteSpriteSheetSprites, spriteSheetLoading } = useSpriteSheet();
  const { spriteSheetImage, spriteSheetImageLoading } = useSpriteSheetImage();

  const deleteSpritesDisabled = useMemo<boolean>(
    () =>
      !spriteSheetImage?.url ||
      spriteSheetImageLoading ||
      spriteSheetLoading ||
  ...
  const deleteSpritesOnClick = useCallback<Func>(() => {
    if (deleteSpritesDisabled) return;
    deleteSpriteSheetSprites(spriteSelection);
  }, [deleteSpriteSheetSprites, deleteSpritesDisabled, spriteSelection]);

  useKeyDown("Backspace", deleteSpritesOnClick);

  return { deleteSpritesDisabled, deleteSpritesOnClick };
}
